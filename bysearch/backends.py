...
class PineconBackend(DataBackend):
    def dataset_upsert(self, dataset: Dataset, upsert_minibatch_size: int = 1000, text_size: int = 100) -> None:
        ...
        for batch_start in range(0, dataset_size, self.upsert_batch_size):
            ...
            metadatas = [{column_names[i]: (row[i][:text_size] if column_names[i] == self.text_column_name else row[i]) for i in range(len(row))} for row in zip(*data_list)]
            to_upsert = list(zip(ids, embeddings, metadatas))
            self.index.upsert(vectors=to_upsert, batch_size=upsert_minibatch_size)

    def __init__(self, dataset: Optional[Dataset] = None, text_column_name: str = None, api_key: str = None, environment: str ='gcp-starter', index_name: str = None, metric: str = 'euclidean', upsert_batch_size: int = 50000) -> None:
        self.text_column_name = text_column_name
    ...
    def search(self, embedding: NDArray[np.float64], k: int, verbose: bool) -> DataFrame:
        ...
        score_dict = {'score':[row['score'] for row in answer['matches']]}
        columns_dict = {column_name: [row['metadata'][column_name] for row in answer['matches']] for column_name in column_names}
        results_dict = score_dict | columns_dict
...
class ChromaBackend(DataBackend):
    ...
    def __init__(self, dataset: Optional[Dataset] = None, text_column_name: str = None, type: str = 'ephemeral', collection_name: str = None, upsert_batch_size: int = 5461, **kwargs) -> None:
        self.text_column_name = text_column_name
        if type == 'ephemeral':
